...
# Beta functions adjusted to match beam sizes: proton beta_x=0.12 m, beta_y=0.06 m; electron beta_x=1.2 m, beta_y=0.05 m.
# Beam currents calculated and added: I_e = N_e * f_coll * e * 1000 (mA), I_p = N_p * f_coll * e (A); I_e ~58 mA (<=60 mA LHeC), I_p=1 A (EIC).
...
# General parameters
factor = 3               # Factor to decrease bunches and increase N_p
num_bunches = 1160 // factor  # Decreased to ~386 to approximate LHeC f_coll ~30 MHz

# Electron beam parameters (from LHeC, energy changed to 10 GeV)
...
eps_n_ey_e = 22e-6       # Normalized vertical emittance (m, assuming round beam as in LHeC)
beta_x_e = 1.2           # Beta function horizontal at IP (m)
beta_y_e = 0.05          # Beta function vertical at IP (m)
bunch_length_e = 0.003   # RMS bunch length (m)

# Proton beam parameters (from EIC 275 GeV, bunch population increased)
energy_p = 275.0         # Proton energy (GeV)
N_p = 6.9e10 * factor    # Proton bunch population (particles per bunch, increased to keep I_p constant)
eps_n_ex_p = 3.3e-6      # Normalized horizontal emittance (m)
eps_n_ey_p = 0.3e-6      # Normalized vertical emittance (m)
beta_x_p = 0.12          # Beta function horizontal at IP (m)
beta_y_p = 0.06          # Beta function vertical at IP (m)
...
print("Hybrid (EIC Proton + LHeC Electron) Beam Parameters and Calculations")
print("------------------------------------")
...
# To modify: Change parameter values for further optimization. For example, adjust beta* for better matching or add hourglass factor: L_cm *= 0.98
...
# With these changes, beam sizes are well matched, currents within limits, and f_coll ~30 MHz close to LHeC.